...
public class NativeBillingClientManager {
     ...
     */
    public static void init(Activity activity, final OnPurchaseCallBack listener) {
    ...
    private static void querySkuDetailsAsync(@BillingClient.SkuType String itemType,
                                             SkuDetailsResponseListener listener, String... skuList) {
        if (billingClient == null)
     ...
     */
    private static BillingResult launchBillingFlow(SkuDetails skuDetails) {
     ...
     */
    public static void consumeAsync(Purchase purchase, ConsumeResponseListener listener) {
        if (billingClient == null)
...
//                    .setDeveloperPayload(purchase.getDeveloperPayload())//指定开发人员有效负载与购买信息一起发回。
     ...
     */
    public static void acknowledgePurchase(Purchase purchase, AcknowledgePurchaseResponseListener listener) {
        if (billingClient == null)
